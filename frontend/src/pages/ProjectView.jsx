...
export default function ProjectView() {
    ...
    // Novos estados para o modal de conclusão de etapa
    ...
    // Função disparada pelo clique no checkbox
    const handleStepClick = (stepId) => {
        ...
        setCompletionNote('');
        setShowCompleteStepDialog(true);
    };

    // Função que finaliza a mutação após o comentário
    const handleConfirmStepCompletion = async () => {
        if (!completionNote.trim() || !selectedStep) return;
    ...
    const handleDeleteProject = async () => {
        setIsDeleting(true);
        try {
            for (const step of steps) { await base44.entities.Step.delete(step.id); }
            for (const objective of objectives) { await base44.entities.Objective.delete(objective.id); }
    ...
    return (
        <div className="min-h-screen bg-[#0a0a0a]">
                    ...
                    {objectives.map((objective, index) => (
                        ...
                        >
                            <ObjectiveAccordion
                                ...
                                onCompleteStep={handleStepClick} // ALTERADO AQUI para abrir o modal
                            />
                        </motion.div>
                    ))}
                </div>
            ...
            {/* Modal de Conclusão de Etapa (Nota Obrigatória) */}
            <AlertDialog open={showCompleteStepDialog} onOpenChange={setShowCompleteStepDialog}>
                <AlertDialogContent className="bg-[#1a1a1a] border-white/10 text-white max-w-md">
                    <AlertDialogHeader>
                        <AlertDialogTitle className="text-xl font-bold">Concluir etapa</AlertDialogTitle>
                        <AlertDialogDescription className="text-gray-400">
                            Descreva brevemente o que foi feito para concluir esta etapa:
                            <div className="mt-2 p-3 bg-white/5 rounded-lg text-white border border-white/10 font-medium">
                                {selectedStep?.title}
                            </div>
                        </AlertDialogDescription>
                    </AlertDialogHeader>
            ...
            <textarea
                ...
                placeholder="Ex: Tarefa finalizada conforme o planejado..."
                className="w-full h-32 bg-[#0a0a0a] border border-white/10 rounded-xl p-4 text-sm focus:ring-2 focus:ring-violet-500 outline-none resize-none transition-all placeholder:text-gray-600"
            />
                    </div>
                    <AlertDialogFooter>
                        <AlertDialogCancel className="bg-transparent border-white/10 text-gray-400 hover:bg-white/5 hover:text-white">
                            Cancelar
                        </AlertDialogCancel>
                        <Button
                            onClick={handleConfirmStepCompletion}
                            disabled={!completionNote.trim() || completeStepMutation.isPending}
                            className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold"
                        >
                            {completeStepMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Confirmar conclusão'}
                        </Button>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
